...
class CaveExplorer(Node):
    def __init__(self):
        ...
        # Variables for Adv. Task 1 - Robust Perception
        self.degradation_mode_ = "normal"
        self.last_degradation_change_ = time.time()
        ...
        # YOLOv8 Implementation from Best.pt (Simplified Ultralytics API)
        try:
    ...
    def map_callback(self, map_msg: OccupancyGrid):
        ...
        if not hasattr(self,
                       "visit_counts_") or self.visit_counts_ is None or self.visit_counts_.shape != self.map_grid_.shape:
            self.visit_counts_ = np.zeros_like(self.map_grid_, dtype=np.float32)

    # Test to confirm YOLO Detection
    #    def test_yolo(self):
    ...
    def image_callback(self, msg):
        ...
        # ADVANCED 1: Robust Perception (10 s visual degradation cycle)
        if not hasattr(self, "degradation_mode_"):
            self.degradation_mode_ = "normal"
            self.last_degradation_change_ = time.time()

        # Cycle every 10 seconds
        if time.time() - self.last_degradation_change_ > 10:
            modes = ["normal", "blur", "dark", "noise"]
            current_idx = modes.index(self.degradation_mode_)
            self.degradation_mode_ = modes[(current_idx + 1) % len(modes)]
            self.last_degradation_change_ = time.time()
            self.get_logger().warn(f" Switched to visual degradation: {self.degradation_mode_}")

        # Apply the selected visual effect
        degraded_image = cv_image.copy()

        if self.degradation_mode_ == "blur":
            degraded_image = cv2.GaussianBlur(degraded_image, (25, 25), 10)
        elif self.degradation_mode_ == "dark":
            degraded_image = cv2.convertScaleAbs(degraded_image, alpha=0.4, beta=-80)
        elif self.degradation_mode_ == "noise":
            # Add strong Gaussian noise to show blur and noise degradation
            noise = np.random.normal(0, 60, degraded_image.shape).astype(np.int16)
            degraded_image = np.clip(degraded_image.astype(np.int16) + noise, 0, 255).astype(np.uint8)

        # Run YOLO detection
        results = self.yolo_model_.predict(degraded_image, imgsz=640, conf=0.3)
        if not results or len(results[0].boxes) == 0:
            return
        ...
        # Draw boxes or publish annotated image
        self.image_detections_pub_.publish(
            self.cv_bridge_.cv2_to_imgmsg(annotated_frame, "bgr8")
        )

        # New Estimate artifact direction (for localisation)
    ...
    def main_loop(self):
        ...
        # keep “trail” avoids reusing same corridors
        ...
        # If we’re already executing a goal
        if not self.ready_for_next_goal_:
            return
        ...
        if self.mode_ == "INSPECT" and self.last_estimated_artifact_ is not None:
            ...
            self.get_logger().info(
                f'Planning 2: standoff -> ({sx:.2f},{sy:.2f}) face θ={math.degrees(theta_to_art):.1f}°')
    ...
    def perform_scan_routine(self):
        ...
        # --- Rotate 90 degrees and save images at each step ---
    ...
    def select_next_frontier_goal(self, items, robot_pose):
        ...
        for (cx, cy, size) in items:
            ...
            score = (
                        1.2 * gain + self.w_size * size - self.w_dist * d_pen - self.w_visit_local * vloc - self.w_visit_path * vpath)
